...
Values are only charted when they are exposed in deck text, embedded chart tables, pasted overlay tables, or raw XLS summary tables. When a supplied deck table differs from the raw XLS, the deck table is used and noted in `data.js`. Q4 2025 unaided awareness is intentionally left blank because the supplied files only show it as an unlabeled chart point.
